...
let allCaps = runners.map(uppercase => {
...
let largeShirts = runners.filter(banana => {
    return banana.shirt_size === "L"
});
...
// Problem 1
// The event director, now HUNGOVER with power, wants to change the last name to upper case for all runners. 
// Map this out for me.
let lastNameCapital = runners.map(function(item){
    return item.last_name.toUpperCase();
})
// console.log(lastNameCapital)
// Problem 2
// For each runner, tell me what company they work for!
let company = [];
